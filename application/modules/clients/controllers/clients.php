...
class Clients extends Admin_Controller {
    ...
    public function status($status = 'active', $page = 0)
    {
		...
		$this->layout->set('meter_type', $this->mdl_clients->meter_type());
		$this->layout->set('connections', $this->mdl_clients->connections());
        ...
        $this->load->model('tariff_type/mdl_tariff_type');
		$tariff_names = $this->mdl_tariff_type->get()->result();
		$this->layout->set('tariff_names', $tariff_names);
    ...
    public function form($id = NULL)
    {
		...
		$this->layout->set(
			array(
				'id' => $id,
				'meter_type' => $this->mdl_clients->meter_type()
			)
		);
		...
		$this->layout->set(
			array(
				'id' => $id,
				'connections' => $this->mdl_clients->connections()
			)
		);
