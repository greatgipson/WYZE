	...
	if($varbmsid>=3){
		...
		while($rows1 = mysql_fetch_array($qry1))
		{
			...
			if($icnt==0){
				...
				$main_query[$icnt]= "select meternum, dates, ((fi_meter_data.activewh/1000)) as column1,(fi_meter_data.activeva/1000) as column2,(fi_meter_data.activevarh/1000) as column3 FROM fi_meter_data where  meternum=".$MeterNumber." and dates > date('".$start_month." 00:30:00') and dates <= date('".$nextMonth." 00:00:00') order by dates";
			}else{
				$main_query[$icnt]= "select meternum, DATE_FORMAT(dates,'".$tYear."-".$tMonth."-%d %H:%i:00') as dates, ((fi_meter_data.activewh/1000)) as column1,(fi_meter_data.activeva/1000) as column2,(fi_meter_data.activevarh/1000) as column3 FROM fi_meter_data where  meternum=".$MeterNumber." and dates > date('".$start_month." 00:30:00') and dates <= date('".$nextMonth." 00:00:00') order by dates";
			}
		...
		if($varbmsid>0){
			...
			if(!empty($week_start_array)){
				...
				for($i = 0; $i < $max;$i++)
				{
					...
					FROM fi_meter_data where  meternum=".$MeterNumber." and dates > date('".trim($year)."-".trim($month)."-".trim($day)." 00:30:00')
					and dates <= date('".trim($year1)."-".trim($month1)."-".trim($day1)." 00:00:00') order by dates";
		...
		<div class="row-fluid">
				<div class="widget">
					<div class="widget-title">
						<h5><?php echo lang('graph_reports'); ?></h5>
