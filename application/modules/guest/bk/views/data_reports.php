		...
		<div class="row-fluid">
				<div class="widget">
						...
						<div class="widget-title">
							<h5><?php echo lang('data_reports'); ?></h5>
							   <form id="myForm" method="POST">
									...
									<?php if($varbmsid >0 and (!empty($varmonthid) OR !empty($varyearid))) { ?>
										<a data-toggle="modal" href="#power-factor-content" class="btn"><i class="icon-eye-open"> </i>Usage Graph </a> &nbsp;&nbsp;&nbsp;&nbsp;
									<?php } ?>
									</td>
									...
									<td>
											 <select id=bmsid name=bmsid onchange="document.forms['changebmsid'].submit()">
												<option value=""></option>
											<?php if($varbmsid==1){ ?>
									...
									<td>
											...
											if($varbmsid<3){
													echo '<select id=monthid name=monthid onchange=\"document.forms[\'changemonthid\'].submit()\"><option value=""></option>';
			...
			if($varbmsid==3 AND !empty($varyearid)){
								...
								$graph1= $graph1."data: [";
										...
										for($m = 1;$m <= 12; $m++){
										{
											...
											dates >= date('".trim($varyearid)."-".trim($m)."-".trim(1)." 00:30:00') and
											dates <= date('".trim($varendyear)."-".trim(($end_month))."-".trim(1)." 00:00:00')
											order by dates";

											//echo $main_query[$kCnt];
			...
			$kWhdaytotal= $kWhdaytotal + ($rows['column1']);
				       ...
				       $graph2= "<script type=\"text/javascript\">
									...
									if($varbmsid>0){
										...
										if(!empty($week_start_array)){
											...
											for ($i = $max-1; $i >=0 ; $i--)
											{
												...
												$main_query[$i]= "select meternum,DATE_FORMAT(dates,'%Y-%m-%d')  as meter_date_format,DATE_FORMAT(dates,'%H:%i:%s') as meter_time_format, dates, ((fi_meter_data.activewh/1000)) as column1,(fi_meter_data.activeva/1000) as column2,(fi_meter_data.activevarh/1000) as column3 FROM fi_meter_data where  meternum=".$MeterNumber." and dates >= date('".trim($year)."-".trim($month)."-".trim($day)." 00:30:00') and dates <= date('".trim($year1)."-".trim($month1)."-".trim($day1)." 00:00:00') order by dates";
											}
						...
						$(function () {
							...
							<div id="container5555" style="min-width: 310px; height: 500px; margin: 0 auto"></div>
