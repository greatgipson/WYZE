...
$QueryString_1="SELECT max(dates) as dates1 FROM fi_meter_data WHERE dates = DATE_ADD((SELECT MAX(dates) FROM fi_meter_data), INTERVAL -1 DAY)";
$current_date = date('Y-m-d h:i:s a', time());

$qry_1 = mysql_query($QueryString_1);
while($rows_1 = mysql_fetch_array($qry_1))
{
	$temp_date = $rows_1['dates1'];;
}

//$temp_date = '2017-09-09 00:30:00';
 
 
$datetime1 = new DateTime($temp_date);
...
$StartMonth = -1;
//$currentday = -2; //-1
$currentday = (-1)*$interval->format('%a');
$yesterday = (-1)*($interval->format('%a')-1); //0
...
function popup(){
var url = "http://localhost:8080/JTUPortal/index.php/guest/quotes/status/open";
...
for($i = 0; $i < $max;$i++)
{
	...
	while($rows = mysql_fetch_array($qry))
	{
		...
		$sec= trim($date_time->format('s'));
			...
			//echo "<pre>".$kWnighttotal."</pre>";
			//echo "<pre>".$kWdaytotal."</pre>";			
...
while($rows = mysql_fetch_array($qry))
{
	...
	$sec= $date_time->format('s');
		...
		if($int==1){
			...
			//$kVADemandArray  = "[[Date.UTC(".$year.", ".$month.", ".$day.",".$hour.",".$min.",".$sec."),".$kVADemand."]"; //Commented on Sep 9 based on Kannan's request
			$kVADemandArray  = "[[Date.UTC(".$year.", ".$month.", ".$day.",".$hour.",".$min.",".$sec."),".($rows['column2'])."]";
			if($PowerFactor>0){
			...
			//$kVADemandArray  = $kVADemandArray.",[Date.UTC(".$year.", ".$month.", ".$day.",".$hour.",".$min.",".$sec."),".$kVADemand."]"; //Commented on Sep 9 based on Kannan's request
			$kVADemandArray  = $kVADemandArray.",[Date.UTC(".$year.", ".$month.", ".$day.",".$hour.",".$min.",".$sec."),".($rows['column2'])."]";
			if($PowerFactor>0){
    ...
    $('#container10').egscharts({
        ...
        series: [{
			...
			//for($i = 0; $i < $max;$i++)
			{
					...
					// if (is_numeric($max_pf_kva[0])) {			
					//echo "<pre>".$i." - ".$max_pf_kva[0]." - Not Numeric</pre>";
					//}else{
					//echo "<pre>".$i." - ".$max_pf_kva[0]."</pre>";
					//} 
...
<div class="container-fluid">
    <div class="row-fluid">
		...
		<div class="card dashboard">
			<div class="card-header">
				<div class="row">
					<div class="col-sm-3">
						<h5 class="card-title mb-0"><?php echo lang('dashboard'); ?></h5>
					</div>
					<div class="col-sm-9 d-none d-md-block">
						<form name="client_meter" method="post">
						<table  style="float:right" class="dashboard">
						  <tr>
							<td>Customer Name: </td>
							<td>
							  <select class="form-control" name="pid" onchange="document.getElementById('clientmeterchanged').value = '1'; document.forms['client_meter'].submit();">
								  <option value=""></option>
								  <?php
							...
							<td>Meter Name: </td>
							<td>
							  <select class="form-control" name="tid" onchange="document.getElementById('clientmeterchanged').value = '1'; document.forms['client_meter'].submit();">
							  <option value=""></option>
							  <?php
					...
					<?php if(strlen($clientid)>0 and strlen($taskid)>0){ ?>
									<div class="float-right">
										<a data-toggle="modal" href="#kWh-kVA-content" class="btn btn-primary"><i class="icon-eye icons"> </i>Daily kWh</a>
										<a data-toggle="modal" href="#kva-demand-content" class="btn btn-primary"><i class="icon-eye icons"> </i>Daily kVA Demand</a>
										<a data-toggle="modal" href="#power-factor-content" class="btn btn-primary"><i class="icon-eye icons"> </i>Daily Power Factor</a>
										<a data-toggle="modal" href="#avg-power-factor-content" class="btn btn-primary"><i class="icon-eye icons"> </i>Daily Average Power Factor</a>
									</div>
					<?php } ?>
	...
	<div class="card-body">	
		<div class="row">
			<div class="col-sm-12">
<?php
//echo $clientid."--".$taskid;
if(strlen($clientid)>0 and strlen($taskid)>0){ ?>
<table border=0 style="margin: auto;"><tr>
<td>
...
<td>
	...
	<div class="col-sm-12">
		<center><h3 class="card-title mb-0"><u>Last 4 Month Usage/Trends</u><h3></center>
	</div>
	<div class="col-sm-6">
	...
	<div id="kWh-kVA-content" class="modal fade">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">	
			<div class="modal-header">
				<h4 class="modal-title">Daily Energy Consumption kWh</h4>
	...
	<div id="power-factor-content" class="modal fade">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">	
			<div class="modal-header">
				<h4 class="modal-title">Daily Power Factor</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				  <span aria-hidden="true">&times;</span>
				</button>				
			</div>
		...
		<div id="avg-power-factor-content" class="modal fade">
		  <div class="modal-dialog" role="document">
			<div class="modal-content">		
				<div class="modal-header">
					<h4 class="modal-title">Daily Average Power Factor</h4>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					  <span aria-hidden="true">&times;</span>
					</button>					
				</div>
		...
		<div id="kva-demand-content" class="modal fade">
		  <div class="modal-dialog" role="document">
			<div class="modal-content">		
				<div class="modal-header">
					<h4 class="modal-title">Daily kVA Demand</h4>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					  <span aria-hidden="true">&times;</span>
					</button>					
				</div>
...
<div class="row">
	<div class="col-sm-12 text-center">
		...
		<div style="margin-top: -29px">
			<h3><img src='<?php echo base_url(); ?>assets/default/img/logo_white.jpg' />
			<span class="lead">Metering Solution</span></h3>
		</div>
		<div class="alert alert-danger" role="alert">
			<h4>Please select Customer Name and Meter Name</h4>
		</div>
	</div>
</div>
<?php } ?>
